  ...
  if (getToken()) {
    ...
    if (to.path === '/login') {
      ...
      NProgress.done()
		...
		if(to.path === '/basicinfo/basicinfo/student'){
			next();
			return;
		}
		if(to.path === '/basicinfo/basicinfo/teacher'){
			next();
			return;
		}
		if(to.path === '/basicinfo/basicinfo/device'){
			next();
			return;
		}
		if(to.path === '/basicinfo/basicinfo/car'){
			next();
			return;
		}
		if(to.path === '/vedio/vedio/manage'){
			next();
			return;
		}
		...
		if(to.path === '/task/task/event'){
			next();
			return;
		}
		if(to.path === '/sysuser/sysuser/role'){
			next();
			return;
		}
		if(to.path === '/task/task/duty'){
			next();
			return;
		}
		if(to.path === '/task/task/visit'){
			next();
			return;
		}
		if(to.path === '/sysuser/sysuser/userinfo'){
			next();
			return;
		}
      ...
      if (store.getters.roles.length === 0) {
        ...
        store.dispatch('GetInfo').then(res => {
			...
			if(to.path === '/vedio/vedio/imdvideo'){
				next();
				return;
			}
			if(to.path === '/vedio/vedio/manage'){
